...
- Действия: вкл/выкл, продление (+1/+30/+90/+180 или дата в формате ДД.ММ.ГГ),
  ...
  - Пресет-продление: если подписка ещё активна — дни прибавляются к текущей
    дате окончания; если истекла (или без срока) — считаются от сегодня.
...
| `PANEL_VERSION` | версия панели: `2.8.0` (по умолч.) или `2.7.4` |

## Версии панели
Код поддерживает и Remnawave **2.8.0**, и **2.7.4** — разница задаётся
переменной `PANEL_VERSION` (влияет на формат запроса `revoke`; трафик и
статистика работают на обеих версиях автоматически).

- Ветка `main` — дефолт `2.8.0`.
- Ветка `panel-2.7.4` — дефолт `2.7.4`.

Достаточно выставить `PANEL_VERSION` в `.env` под свою панель; ветки отличаются
только значением по умолчанию.
